...
def _playlist_id(s, name: str) -> str:
    """Id of the named playlist, creating it only if the listing succeeded and it
    genuinely doesn't exist. A transient listing error used to fall through to
    create_playlist, which produced a SECOND playlist with the same name."""
    pid = _playlist_id_if_exists(s, name)     # raises if it couldn't be checked
    if pid:
        return pid
    pl = s.user.create_playlist(name, "Auto-generated by musicguru")
    log.info("Tidal: created playlist %r", name)
    _pl_cache[name] = pl.id
    _remember_id(name, pl.id)
    return pl.id
...
def playlist_membership(name: str) -> set:
    ...
    pid = _playlist_id_if_exists(s, name)   # raises if Tidal couldn't be asked
    if not pid:
        return set()                        # genuinely no such playlist yet
    keys = set()
    try:
        ...
        # Half a page of results is worse than admitting we don't know.
        raise TidalUnavailable(f"membership read failed: {e}")
    return keys


def _list_playlists(s) -> list:
    """User playlists. tidalapi asks for limit=1000 in one go and Tidal has been
    500ing on that, so page it in small chunks first and only fall back to the
    library's own call if paging isn't supported."""
    try:
        out, offset = [], 0
        while True:
            batch = list(s.user.playlists(limit=50, offset=offset))
            out.extend(batch)
            if len(batch) < 50:
                return out
            offset += 50
        ...
        pass                             # this tidalapi doesn't take limit/offset
    except Exception as e:
        log.debug("Tidal paged playlist listing failed: %s", e)
    return list(s.user.playlists())


def _is_missing(e: Exception) -> bool:
    """A 404/not-found, as opposed to a transient server error."""
    txt = f"{e}".lower()
    return "404" in txt or "not found" in txt


def _remembered_id(name: str):
    try:
        from ..storage.db import state_get
        return state_get(f"tidal.playlist.{name}")
    except Exception:
        return None


def _remember_id(name: str, pid: str) -> None:
    try:
        from ..storage.db import state_set
        state_set(f"tidal.playlist.{name}", str(pid))
    except Exception:
        pass


def forget_playlist_id(name: str) -> None:
    _pl_cache.pop(name, None)
    try:
        from ..storage.db import state_del
        state_del(f"tidal.playlist.{name}")
    except Exception:
        pass
...
def _playlist_id_if_exists(s, name: str):
    ...
    TidalUnavailable if the listing itself failed -- "we couldn't ask" must not
    be mistaken for "it doesn't exist", or we'd create a duplicate playlist.

    A previously resolved id is remembered across restarts, so a listing outage
    (Tidal 500s on this endpoint) doesn't stop us adding to a playlist we already
    know the id of.
    """
    if name in _pl_cache:
        return _pl_cache[name]
    pid = _remembered_id(name)
    if pid:
        _pl_cache[name] = pid
        return pid
    try:
        playlists = _list_playlists(s)
    except Exception as e:
        raise TidalUnavailable(f"playlist listing failed: {e}")
    for pl in playlists:
...
def add_to_named_playlist(name: str, artist: str, title: str, album: str = None) -> str:
    ...
    try:
        ...
        if _is_missing(e):
            # The remembered playlist is gone (deleted on Tidal). Forget the id
            # and resolve again rather than failing forever against a dead id.
            log.info("Tidal playlist %r no longer exists; re-resolving", name)
            forget_playlist_id(name)
            _pl_tracks.pop(pid, None)
            pid = _playlist_id(s, name)
            _fresh_playlist(s, pid).add([tid])
        else:
