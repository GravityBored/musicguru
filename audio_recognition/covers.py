...
Files are re-encoded to JPEG and capped at COVER_MAX_PX, so ~10k covers costs
...
def path_for(url: str) -> str:
    return os.path.join(COVER_CACHE_DIR, key(url) + ".jpg")


def cached(url: str) -> str | None:
    if not url or not COVER_CACHE_ENABLED:
        return None
    p = path_for(url)
...
def _encode(data: bytes) -> bytes | None:
    ...
    try:
        ...
        if img.mode != "RGB":
            img = img.convert("RGB")
        img.thumbnail((COVER_MAX_PX, COVER_MAX_PX), Image.LANCZOS)
        buf = BytesIO()
...
def _write_disk(dest: str, jpeg: bytes) -> None:
    ...
    fd, tmp = tempfile.mkstemp(dir=COVER_CACHE_DIR, suffix=".jpg")
    os.close(fd)
    try:
...
def store_bytes(url: str, data: bytes) -> str | None:
    ...
    if not url or not data or not COVER_CACHE_ENABLED:
        return None
    dest = path_for(url)
    on_disk = os.path.exists(dest) and os.path.getsize(dest) > 0
    if on_disk and (not COVER_DB_ENABLED or db.has_cover_blob(key(url))):
        return dest
    ...
    try:
        ...
        for chunk in r.iter_content(64 * 1024):
            ...
            if buf.tell() > IMAGE_MAX_BYTES:
                log.warning("Cover exceeds %d bytes; dropping", IMAGE_MAX_BYTES)
                return None
        return buf.getvalue()
